...
| DD-015 | 2026-06-03 | Data model | Human-readable `key` (INIT-/PRJ-/TASK-) assigned by creation order in the seed | Lets users reference and track items quickly without long cuids | New client items compute the next PRJ-n locally |
| DD-016 | 2026-06-03 | App shell | Encode the active view in the URL (`?view=calendar`) instead of only component state | Makes the chosen view shareable via the link, even on a static host | Read on mount via `window.location` to avoid Suspense in static export |
| DD-017 | 2026-06-03 | Product principle | Treat **deadlines as commitments** and **alignment as a requirement** — make both visible on every project (deadline countdown, time-elapsed-vs-work-done, "Unaligned" flag) | Owner's two core principles: time/date is a commitment; every project must ladder to a business initiative to be meaningful | Projects with no initiative are flagged; behind-schedule is shown up front |
| DD-018 | 2026-06-03 | App shell | Forms use **native HTML controls + Tailwind** (shared primitives in `Modal.tsx`), not a component library; native browser chrome is normalized to the dark theme | Minimal deps, full control over look, consistent styling | Custom `Select` chevron + `NumberStepper`; number spinner hidden, date indicator tinted (`globals.css`) |
